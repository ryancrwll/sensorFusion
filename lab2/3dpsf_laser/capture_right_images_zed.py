...
def capture_right_images(port, out_dir, continous):
    ...
    while True :
        ...
        if k == 27: # ESC key, exit
            ...
            cv2.imwrite(os.path.join(out_dir, right_img_name), left_right_image[1])
            image_id = image_id+1
            print("- Captured right image " + str(image_id))           
    exit(0)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Captures images from the right camera of a Zed2 stereo")
    ...
    capture_right_images(params.port, params.out_dir, params.continous)